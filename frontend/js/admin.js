...
function initDashboard() {
  ...
  if (window.QRCode) {
    ...
    new QRCode(qrHolder, { text: link, width: 160, height: 160 });
  }

  servingBoard = new FlapBoard(document.getElementById('serving-flaps'), 3);
...
async function refresh() {
  ...
  try {
    ...
    clearError();
    servingBoard.set(data.restaurant.currentServing);
    ...
    data.entries.forEach((entry) => {
      ...
      tr.innerHTML = `
        ...
        <td class="row-actions">
          ...
          <button class="secondary" data-action="cancel" data-token="${entry.token}">Cancel</button>
        </td>
      `;
